<template>
  <div class="main_box">
    <div class="box">
      <div class="header">
        <div v-for="(item, index) in tab" :key="index" @click="changeNav(index)"
          :class="tabIndex == index ? 'active-color' : ''">
          {{ item }}
        </div>
      </div>
      <div class="context">
        <div v-show="tabIndex == 0" v-for="item in songSheets" :key="item.id"
...
export default {
  ...
  methods: {
    ...
    async goSearch(keyword) {
      const { data: res } = await Common.getCommonSearch(keyword);
      if (res.code == 1) {
        this.singers = res.data.singers;
        this.songs = res.data.songs;
        this.songSheets = res.data.songSheets;
